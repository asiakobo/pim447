...
static void pim447_process_movement(struct pimoroni_pim447_data *data, int delta_x, int delta_y, float smoothing_factor)
{
    ...
    delta_x = (delta_x < 0) ? -(delta_x * delta_x) : delta_x * delta_x;
    delta_y = (delta_y < 0) ? -(delta_y * delta_y) : delta_y * delta_y;

    int scaled_x_movement = (int)(delta_x * scaling_factor * 128) >> 7;
    int scaled_y_movement = (int)(delta_y * scaling_factor * 128) >> 7;
...
static void pimoroni_pim447_gpio_callback(const struct device *port, struct gpio_callback *cb, gpio_port_pins_t pins)
{
    ...
    LOG_DBG("GPIO callback fired. Pins: 0x%08lX, %d expected bit: 0x%08lX", pins, config->int_gpio.pin, BIT(config->int_gpio.pin));
...
static int pimoroni_pim447_enable(const struct device *dev)
{
    ...
    LOG_INF("Configured GPIO pin: %d, bitmask: 0x%08lX", config->int_gpio.pin, BIT(config->int_gpio.pin));
